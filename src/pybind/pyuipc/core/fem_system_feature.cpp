...
PyFEMSystemFeature::PyFEMSystemFeature(py::module& m)
{
    auto class_FEMSystemFeature =
        py::class_<FEMSystemFeature, IFeature, S<FEMSystemFeature>>(
            m, "FEMSystemFeature", R"(Feature for computing contact energy, gradients, and Hessians.)");

    class_FEMSystemFeature.def(
        "contact_energy",
        [](FEMSystemFeature& self, U64 uid, geometry::Geometry& energy)
        { self.contact_energy(uid, energy); },
        py::arg("uid"),
        py::arg("energy"),
...
Args:
    ...
    class_FEMSystemFeature.def(
        ...
        { self.contact_gradient(uid, vert_grad); },
        py::arg("uid"),
        py::arg("vert_grad"),
...
Args:
    ...
    class_FEMSystemFeature.def(
        ...
        { self.contact_hessian(uid, vert_hess); },
        py::arg("uid"),
        py::arg("vert_hess"),
